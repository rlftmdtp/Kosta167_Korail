...
<%
	...
	if(station_map !=null){
		m_list = service.station_searchService(request);
		session.setAttribute("station_list", m_list);
		System.out.println("*메인m_list : " + m_list);
		System.out.println("*메인 s_map : " + station_map);
	}
%>
...
<script type="text/javascript"	src="../../javascript/partner_javascript/Partner_stationPage.js"></script>
	
<link rel="stylesheet" href="../../css/partner_css/partner_station.css?ver=1"	type="text/css" />
...
<!-- 여기서부터 내 코드 -->
	...
	<div class="Partner_station_wrap">
		...
		<hr>
			<ul class="Partner_station_searchList">
				<c:choose >
				<c:when test="${station_list != null}">
					<c:forEach var="s" items="${station_list}">
					<li>
						<a href="#">
							<div>
								<div id="Partner_station_searchImage">
								...
								<div class="Partner_info_cSearch">
									<p class="name">${s.m_name}</p>
									<p class="gender">${s.m_gender}</p>
									<p class="age">${s.m_age}</p>
								</div>
							</div>
						</a>
					</li>
